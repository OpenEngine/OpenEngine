...
1. You choose a graph entry, type your task and repository, and expand
   ...
   **least-utilized** reads utilization when the run starts and picks the
   runner with the most headroom (alternating when no runner could be read),
   and **round-robin** alternates runners per stage; both resolve to a
   concrete runner when the run starts.
