"""What the run pages display, and where it comes from.
...
* `EmptyReadModel` -- the honest one. No runs have been recorded, so there is
  nothing to show.
...
The third arrives when runs are recorded and reads through
`engine.ports.StateStore`. No page changes when it does.

The chat page does not use any of this: a conversation is not a projection of a
run, and it reads through `engine.runtime.AgentSession` instead.
"""
...
class ReadModel(Protocol):
    """Everything the run pages are allowed to know.
...
class EmptyReadModel:
    """Nothing to show, because nothing records runs yet. Implements `ReadModel`."""
