...
These are the tickets for that, in dependency order. **T1–T4 are prerequisites**
and buy nothing on their own; **T5–T12 are the behaviours**; **T13 is a product
...
below were checked against the tree at `be7b91b`; if a line number has moved,
the symbol name is the durable half.

| | ticket | depends on |
| --- | --- | --- |
| T1 | Make a run reviewable without re-running it | — |
| T2 | Give the harness an `origin` to base runs on | — |
| T3 | Teach the fake CLIs to call MCP tools | — |
| T4 | A `gh` that is not GitHub | — |
| T5 | Provision → implement → `complete_step` → review | T2, T3 |
| T6 | An approval inside a workflow conversation | T5 |
| T7 | A question reaches the operator and resumes the step | T5 |
| T8 | The reviewer adds review comments | T5, T4 |
| T9 | Talking after review reopens implementation | T5 |
| T10 | Auto-approve clears several requests unattended | T5 |
| T11 | A failed run reads as failed | T5 |
| T12 | A plan reaches the operator | T5 |
| T13 | Nothing in the client finishes a human review | — (product) |

---

## T1 — Make a run reviewable without re-running it

**Why.** There is no artifact of a green run today. Traces are kept on failure
only, CI uploads them on failure only, and there is no report to open. Somebody
asking "what does the approval card look like when it arrives?" has to run the
suite themselves.

Traces rather than video: a Playwright trace is a scrubbable timeline with a
live DOM snapshot at every step, so the approval card can be *inspected* in the
replay. Video is a flat recording that says less.

**Work.**

1. `apps/web/playwright.config.ts`
   - `reporter`: add `["html", { open: "never" }]` to both the CI and the local
     array. Keep `["list"]` so the terminal still says something useful.
   - `use.trace`: `process.env.CI ? "retain-on-failure" : "on"`. A trace of two
     passing tests costs nothing locally, and a trace of *the run somebody is
     asking about* is worth more than one of the run that happened to fail.
   - Leave `retries: 0` and the per-test server model alone.
2. A screenshot helper, in `e2e/harness.ts` or a sibling `e2e/evidence.ts`:
   `await shot(page, testInfo, "approval pending")` takes a full-page PNG and
   attaches it to the report under that name. Call it in
   `chat-approvals.spec.ts` at the three states it already asserts on --
   streaming mid-turn, the pending approval card, the decided card. Name them so
   the report reads as a sequence.
   These are documentation as much as diagnostics: every ticket below should
   attach one still at its decisive moment, and T5–T12 say where.
3. `.github/workflows/tests.yml`, `browser` job: upload
   `apps/web/playwright-report` with `if: always()`, keeping the existing
   failure-only `test-results` upload for raw traces. Say in the step name that
   the artifact is a static site -- downloaded, unzipped, and opened with
   `npx playwright show-report <dir>`, because GitHub will not serve it.
4. `.gitignore` already has `test-results/` and `playwright-report/`. Confirm.

**Done when** a green local run leaves a report whose every test has three named
stills and a trace, and `npx playwright show-trace` on a *passing* test replays
the approval click.
...
**Why.** This is the largest prerequisite and it unblocks T5 through T12.
...
an inline one. T8 needs to assert on those calls without touching GitHub.
...
## T5 — Provision → implement → `complete_step` → review

Covers **1a**, **1b**, and **1e** of the original list. Depends on T2, T3.
...
* **Provisioning (1a).** The run page moves off `pending` through
  ...
  path exists on disk. Attach a still here.
...
* **Advancing (1e).** After `complete_step`, the implementation step shows its
  ...
  run reaches `awaiting_human_review` with the "Action required" callout. Attach
  a still of the finished timeline.
...
## T6 — An approval inside a workflow conversation

Covers **1c**. Depends on T5.
...
`complete_step`, and **the file exists in the run's worktree**. Attach stills
either side of the click.

Worth its own spec rather than folding into T5: the decision travels a different
...
## T7 — A question reaches the operator and resumes the step

Covers **1d**. Depends on T5.
...
the assertion is on disk rather than on a protocol frame. Attach a still of the
modal.
...
## T8 — The reviewer adds review comments

Covers **1f**. Depends on T5 and T4.
...
## T9 — Talking after review reopens implementation

Covers extra behaviour **1**. Depends on T5.
...
**Assert.** Drive a run to `awaiting_human_review` (T5), open the implementation
...
again with the *same* review conversation carrying both rounds. Attach a still of
the reopened run.
...
## T10 — Auto-approve clears several requests unattended

Covers extra behaviour **2**. Depends on T5.
...
parks. Attach a still of the settled list -- several `.approval-decided` cards in
a row is exactly the screenshot somebody wants of this feature.
...
## T11 — A failed run reads as failed

Covers extra behaviour **3**. Depends on T5.
...
summary is on the page rather than only in the log. Attach a still of each.
...
## T12 — A plan reaches the operator

Covers extra behaviour **4**. Depends on T5. **Claude only.**
...
approving it lets the turn continue, and rejecting it does not. Attach a still --
this is the card most people will never have seen.

---

## T13 — Nothing in the client finishes a human review
...
T5 is written, because the answer changes what "the end of a run" means for the
whole suite.
