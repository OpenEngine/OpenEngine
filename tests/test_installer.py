...
class InstallerTests(unittest.TestCase):
    def test_engine_is_the_only_installed_launcher(self):
        for legacy in (None, "installer", "unrelated"):
            with self.subTest(legacy=legacy), tempfile.TemporaryDirectory() as directory:
                ...
                bin_dir = root / "bin"
                bin_dir.mkdir()
                old_launcher = bin_dir / "openengine"
                old_contents = "#!/bin/sh\n"
                if legacy == "installer":
                    old_contents += "# Written by the OpenEngine installer; rerunning it rewrites this file.\n"
                if legacy:
                    old_launcher.write_text(old_contents)
                ...
                for _ in range(2):
                    ...
                    if legacy == "unrelated":
                        assert old_launcher.read_text() == old_contents
                    else:
