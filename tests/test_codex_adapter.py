...
#: Shapes generated by ``codex app-server generate-json-schema`` from 0.144.4.
#: The values are compact fixtures; unlike the exec transcripts above they do
#: not claim to be a captured model run.
APP_SERVER_TRANSCRIPT = """\
{"id":0,"result":{"userAgent":"codex","platformFamily":"unix",\
"platformOs":"macos","codexHome":"/tmp/codex"}}
{"method":"thread/started","params":{"thread":{"id":"thr_app"}}}
{"id":1,"result":{"thread":{"id":"thr_app"}}}
{"id":2,"result":{"turn":{"id":"turn_app","items":[],"status":"inProgress"}}}
{"method":"item/started","params":{"threadId":"thr_app","turnId":"turn_app",\
"startedAtMs":1,"item":{"id":"item_1","type":"commandExecution",\
"command":"ls packages","commandActions":[],"cwd":"/repo",\
"status":"inProgress","aggregatedOutput":null,"exitCode":null}}}
{"method":"item/completed","params":{"threadId":"thr_app","turnId":"turn_app",\
"completedAtMs":2,"item":{"id":"item_1","type":"commandExecution",\
"command":"ls packages","commandActions":[],"cwd":"/repo",\
"status":"completed","aggregatedOutput":"adapters\\nports\\n","exitCode":0}}}
{"method":"item/completed","params":{"threadId":"thr_app","turnId":"turn_app",\
"completedAtMs":3,"item":{"id":"item_2","type":"agentMessage","text":"DONE"}}}
{"method":"thread/tokenUsage/updated","params":{"threadId":"thr_app",\
"turnId":"turn_app","tokenUsage":{"last":{"inputTokens":120,\
"cachedInputTokens":80,"outputTokens":12,"reasoningOutputTokens":4,\
"totalTokens":132},"total":{"inputTokens":120,"cachedInputTokens":80,\
"outputTokens":12,"reasoningOutputTokens":4,"totalTokens":132}}}}
{"method":"turn/completed","params":{"threadId":"thr_app",\
"turn":{"id":"turn_app","items":[],"status":"completed"}}}
"""
...
def test_app_server_messages_have_the_same_turn_shape() -> None:
    turn = turn_from_events(app_server_events(parse_events(APP_SERVER_TRANSCRIPT)))

    assert turn.message.content == "DONE"
    call, result = turn.steps
    assert call.tool_calls[0].name == "command_execution"
    assert result.tool_call_id == call.tool_calls[0].call_id
    assert result.content == "adapters\nports\n\n(exit 0)"
    assert turn.usage is not None
    assert turn.usage.prompt_tokens == 120
    assert turn.usage.cached_prompt_tokens == 80
    assert turn.usage.completion_tokens == 12
...
def test_app_server_command_line_is_inspectable() -> None:
    assert CodexAppServerAgentRunner().command_line(PROFILE) == ["codex", "app-server"]
...
def _fake_app_server(tmp_path) -> tuple[str, Path]:
    """A protocol peer that waits for EOF after emitting ``turn/completed``."""
    binary = tmp_path / "codex-app-server"
    requests = tmp_path / "requests.json"
    binary.write_text(
        "#!/usr/bin/env python3\n"
        "import json, sys\n"
        f"requests_path = {str(requests)!r}\n"
        "received = [json.loads(sys.stdin.readline()) for _ in range(3)]\n"
        'print(json.dumps({"id": 0, "result": {}}), flush=True)\n'
        'print(json.dumps({"method": "thread/started", "params": '
        '{"thread": {"id": "thr_app"}}}), flush=True)\n'
        'print(json.dumps({"id": 1, "result": '
        '{"thread": {"id": "thr_app"}}}), flush=True)\n'
        "received.append(json.loads(sys.stdin.readline()))\n"
        "with open(requests_path, 'w') as stream:\n"
        "    json.dump(received, stream)\n"
        f"for line in {APP_SERVER_TRANSCRIPT!r}.splitlines():\n"
        "    message = json.loads(line)\n"
        "    if message.get('id') == 2 or message.get('method', '').startswith('item/') "
        "or message.get('method') in {'thread/tokenUsage/updated', 'turn/completed'}:\n"
        "        print(json.dumps(message), flush=True)\n"
        "assert sys.stdin.readline() == ''\n"
    )
    binary.chmod(0o755)
    return str(binary), requests


def test_app_server_runs_a_turn_with_stdin_open_until_completion(tmp_path) -> None:
    binary, requests_path = _fake_app_server(tmp_path)
    ...
    assert thread_params["developerInstructions"] == "You are terse."
    assert thread_params["model"] == "gpt-test"
    turn_text = requests[3]["params"]["input"][0]["text"]
    assert turn_text == "# Conversation\n\nUser: go"
